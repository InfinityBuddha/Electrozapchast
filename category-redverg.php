<?php
/*
  Template Name: RedVerg Template
*/
        ...
        <div class="catalog__container">
          ...
          <div class="catalog__brands">
            <div class="catalog__row">
                  <?php
                      $args = array( 'post_type' => 'product', 'posts_per_page' => 48, 'product_cat' => 'redverg', 'orderby' => 'price' );
                      $loop = new WP_Query( $args );
                      while ( $loop->have_posts() ) : $loop->the_post(); global $product; ?>
                        <?php woocommerce_show_product_sale_flash( $post, $product ); ?>
                        <div class="catalog__brand catalog__brand--card">
                          <h3 class="catalog__brand-name catalog__brand-name--card"><?php the_title(); ?></h3>
                          <div class="catalog__photo-container">
                            <?php if (has_post_thumbnail( $loop->post->ID )) echo get_the_post_thumbnail($loop->post->ID, 'shop_catalog'); else echo '<img src="'.woocommerce_placeholder_img_src().'" alt="Placeholder"/>'; ?>
                          </div>
                          <div class="catalog__brand-price"><?php echo $product->get_price(); ?> руб</div>
                        </div>
                  <?php endwhile; ?>
                  <?php wp_reset_query(); ?>
            </div>
